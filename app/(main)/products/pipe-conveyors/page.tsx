...
import { CheckCircle2, Cog, Factory, Leaf } from 'lucide-react';
import Link from 'next/link';
import { motion } from 'framer-motion';
import CenterFocusCarousel from '@/components/sections/products/CenterFocusCarousel';
import { pipeConveyorOverview } from '@/data/spareng-products/pipe-conveyors-overview';

export default function PipeConveyorsPage() {
      ...
      <section ref={contentRef} className="py-12 sm:py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="max-w-7xl mx-auto space-y-8 sm:space-y-12">
            {/* Carousel */}
            {pipeConveyorOverview.images.length > 0 && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.2 }}
              >
                <CenterFocusCarousel images={pipeConveyorOverview.images} alt="Pipe Conveyors" />
              </motion.div>
            )}
            ...
            >
              ...
              <div className="prose prose-lg max-w-none text-gray-700 leading-relaxed">
                <p className="text-base sm:text-lg">{pipeConveyorOverview.description}</p>
              </div>
            </motion.div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
              ...
              >
                ...
                <ul className="space-y-3">
                  {pipeConveyorOverview.keyFeatures.map((feature, idx) => (
                    <motion.li
                      key={idx}
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {pipeConveyorOverview.applications.map((app: { title: string; description: string }, idx: number) => (
                    <motion.div
                      key={idx}
                      initial={{ opacity: 0, y: 20 }}
                      animate={{ opacity: 1, y: 0 }}
            ...
            <motion.div
              ...
              className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sm:p-8 lg:p-10"
            >
              <h2 className="text-2xl sm:text-3xl font-bold text-primary-900 mb-8 flex items-center">
                <Leaf className="w-7 h-7 mr-3 text-primary-600" />
                Environmental Advantages
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {pipeConveyorOverview.advantages.map((advantage, idx) => (
                  <motion.div
                    key={idx}
                  ...
                  >
                    <h3 className="text-lg font-bold text-primary-900 mb-3">{advantage.title}</h3>
            ...
            >
              <h2 className="text-2xl sm:text-3xl font-bold mb-4">
                Need Enclosed Pipe Conveyor Solutions?
              </h2>
              <p className="text-lg text-white/90 mb-8 max-w-2xl mx-auto">
                Our team can design environmentally-friendly pipe conveyor systems with complete material containment for your facility.
              </p>
              <Link
                href="/contact"
