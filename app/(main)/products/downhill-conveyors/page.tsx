...
import { CheckCircle2, Cog, Factory, Shield } from 'lucide-react';
import Link from 'next/link';
import { motion } from 'framer-motion';
import CenterFocusCarousel from '@/components/sections/products/CenterFocusCarousel';
import { downhillConveyorOverview } from '@/data/spareng-products/downhill-conveyors-overview';

export default function DownhillConveyorsPage() {
      ...
      <section className="bg-gradient-to-br from-primary-900 via-primary-800 to-primary-900 text-white py-12 sm:py-16">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            ...
            >
              {downhillConveyorOverview.title}
            </motion.h1>
            <motion.p
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              {downhillConveyorOverview.heading}
            </motion.p>
          </div>
        </div>
      </section>
      ...
      <section ref={contentRef} className="py-12 sm:py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="max-w-7xl mx-auto space-y-8 sm:space-y-12">
            {/* Carousel */}
            {downhillConveyorOverview.images.length > 0 && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.2 }}
              >
                <CenterFocusCarousel images={downhillConveyorOverview.images} alt="Downhill Conveyors" />
              </motion.div>
            )}
            ...
            >
              ...
              <div className="prose prose-lg max-w-none text-gray-700 leading-relaxed">
                <p className="text-base sm:text-lg">{downhillConveyorOverview.description}</p>
              </div>
            </motion.div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
              ...
              >
                ...
                <ul className="space-y-3">
                  {downhillConveyorOverview.keyFeatures.map((feature, idx) => (
                    <motion.li
                      key={idx}
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {downhillConveyorOverview.applications.map((app: { title: string; description: string }, idx: number) => (
                    <motion.div
                      key={idx}
                      initial={{ opacity: 0, y: 20 }}
                      animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {downhillConveyorOverview.safetyFeatures.map((feature, idx) => (
                  <motion.div
                    key={idx}
            ...
            >
              <h2 className="text-2xl sm:text-3xl font-bold mb-4">
                Need Custom Downhill Conveyor Solutions?
              </h2>
              <p className="text-lg text-white/90 mb-8 max-w-2xl mx-auto">
                Our engineering team specializes in designing safe and efficient downhill conveying systems with regenerative braking.
              </p>
              <Link
                href="/contact"
